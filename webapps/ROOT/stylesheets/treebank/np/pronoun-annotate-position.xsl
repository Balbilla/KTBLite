...
<xsl:stylesheet xmlns:tb="https://papygreek.hum.helsinki.fi/py/"
    ...
    <xsl:template match="word[@np-head='true' and normalize-space(@pronominal-modifiers)]">
        ...
        <xsl:copy>
            <xsl:variable name="pronominal-modifier-positions" as="xs:string*">
                <xsl:for-each select="tokenize(@pronominal-modifiers, '\s+')">
                    <xsl:value-of select="tb:get-np-modifier-position($head, xs:integer(.))"/>                    
                </xsl:for-each>                
            </xsl:variable>
            <xsl:attribute name="pronominal-modifier-positions" select="$pronominal-modifier-positions"/>
