...
<xsl:stylesheet xmlns:tb="https://papygreek.hum.helsinki.fi/py/"
    ...
    <!-- This stylesheet annotates the quantifier for its position in the NP -->
    
    <xsl:include href="../utils.xsl"/>
    
    <xsl:template match="word[tb:is-quantifier(.)
        and @relation=('ATR', 'ATR_CO', 'APOS', 'APOS_CO')]">
        <xsl:copy>
            <xsl:variable name="head" select="tb:get-governing-substantive(.)"/>
            <xsl:if test="@id = tokenize($head/@quantifier-modifiers, '\s+')">
