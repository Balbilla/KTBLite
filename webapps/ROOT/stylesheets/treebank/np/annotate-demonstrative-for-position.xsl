...
<xsl:stylesheet xmlns:tb="https://papygreek.hum.helsinki.fi/py/"
    ...
    <!-- This stylesheet annotates the demonstrative for its position in the NP -->
    
    <xsl:include href="../utils.xsl"/>
    
    <xsl:template match="word[tb:is-demonstrative-adjective(.)
        and @relation=('ATR', 'ATR_CO', 'APOS', 'APOS_CO')]">
        <xsl:copy>
            <xsl:variable name="head" select="tb:get-governing-substantive(.)"/>
            <xsl:if test="@id = tokenize($head/@demonstrative-modifiers, '\s+')">
