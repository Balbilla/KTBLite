...
<xsl:stylesheet xmlns:tb="https://papygreek.hum.helsinki.fi/py/"
  ...
  <xsl:template match="aggregation">
    <result text="{$text}">
      <xsl:variable name="passed" select="deep-equal(*[1]/*, *[2]/*)" />
      <xsl:attribute name="pass" select="$passed" />
      <xsl:if test="not($passed)">
        <kiln:actual>
          <xsl:apply-templates mode="compare" select="*[1]/*">
            <xsl:with-param name="former-other-node" select="*[2]" />
          </xsl:apply-templates>
        </kiln:actual>
        <kiln:expected>
          <xsl:apply-templates mode="compare" select="*[2]/*">
            <xsl:with-param name="former-other-node" select="*[1]" />
          </xsl:apply-templates>
        </kiln:expected>
      </xsl:if>
    </result>    
