...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0"  
  ...
  <xsl:template match="lst[@name='facet_fields']" mode="search-results">
    <xsl:if test="lst/int">
      ...
      <ul data-uk-accordion="">
        ...
        <li>
          <xsl:call-template name="display-facet-group">
            <xsl:with-param name="fields" select="$subject_fields"/>
            <xsl:with-param name="title" select="'Subject'"/>
          </xsl:call-template>
        </li>
        <li>
          <xsl:call-template name="display-facet-group">
            <xsl:with-param name="fields" select="$object_fields"/>
            <xsl:with-param name="title" select="'Object'"/>
          </xsl:call-template>
        </li>
        <li>
          <xsl:call-template name="display-facet-group">
            <xsl:with-param name="fields" select="$subordination_fields"/>
            <xsl:with-param name="title" select="'Subordination'"/>
          </xsl:call-template>
        </li>
        <li>
          <xsl:call-template name="display-facet-group">
            <xsl:with-param name="fields" select="$order_fields"/>
            <xsl:with-param name="title" select="'Order patters'"/>
          </xsl:call-template>
        </li>
      </ul>
    </xsl:if>
  ...
  <xsl:template match="result/doc" mode="search-results" priority="10">
    ...
    <tr>
      ...
      <td>
        <xsl:value-of select="int[@name='sentence_real_token_count']"/>
      </td>
      <td>
      ...
      <td>
        <ul class="uk-list">
          <xsl:for-each select="arr[@name='sentence_b_w_o_pattern']/str">
            <li><xsl:value-of select="."/></li>
          </xsl:for-each>
        </ul>
      </td>
      <td>
        <xsl:value-of select="bool[@name='sentence_verb_final']"/>
      </td> 
    </tr>
  </xsl:template>
        ...
        <table class="tablesorter uk-hidden">
          <thead>
            <tr>
              ...
              <th scope="col">Real tokens count</th>
              <th scope="col">Text</th>
              <th scope="col">Genre</th>
              <th scope="col">Century</th>
              <th scope="col">Corpus</th>
              <th scope="col">General word order pattern</th>
              <th scope="col">Verb final</th>
            </tr>
          </thead>
          <tfoot>
            <tr>
