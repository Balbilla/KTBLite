...
<xsl:stylesheet version="2.0"
  ...
  <!-- Transforms XML specifying Solr query parameters into an
       ...
       * facet field names, which can take a variety of attributes:

          * a "join" attribute value of "and" or "or", used to
          determine whether this facet's values are ANDed or ORed
          together.

          * a "sort" attribute value of "count" or "index".

          * other attributes based on the parameters available for
          facets, used to provide facet field specific values; eg,
          limit, mincount, missing, etc.
  ...
  <xsl:template match="query">
    <xsl:variable name="query-string-parts" as="xs:string*">
      <!-- This may look strange, but it is designed so that each
           ...
           together. -->
      <xsl:for-each select="* | facet.field/@*">
  ...
  <xsl:template match="q">
    <!-- q parameters are bundled together and handled by the first q
         element, so do not process this at all if it is preceded by a
         q. -->
  ...
  <xsl:template match="facet.field/@*">
    <xsl:text>f.</xsl:text>
    <xsl:value-of select=".." />
  ...
  <xsl:template match="facet.field/@join" priority="10"/>
